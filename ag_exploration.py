...
def grade_population(population, data_file_name, score_method, filter_strat, multithreading):
	"""
	...
	-> filter_strat is a string, name of the filter aplly n cohorte,
	   could be:
	   			- random
	   			- any disesae (SLE, SjS ...)
	-> multithreading is a boolean = True for multithreading, 
	   False for sequential analysis
	...
	if(multithreading == True):
		...
		# => prepare index files for each processor
		for individual in population:
			shutil.copy("DATA\\patientIndex.csv", "DATA\\patientIndex_"+str(individual._id)+".csv")

		# => Use one processor / individual	
		score_list = []
		score_to_individuals = {}	
		num_cores = multiprocessing.cpu_count()
		tup_list = Parallel(n_jobs=num_cores)(delayed(evaluate_individual)(individual, data_file_name, score_method, filter_strat) for individual in population)
...
if __name__ == '__main__':
	...
	"""
	time_file = open("time_score.log", "w")
	time_file.close()
	...
	max_iteration = 10
	number_of_individual_per_generation = 30
	x = 1
	data_file = "DATA\\MATRIX\\panel_"+str(x)+"_filtered_processed.txt"
	run_ag_exploration(data_file, number_of_individual_per_generation, max_iteration, "svm", "random")
	"""
	...
	for x in range(1,7):
		...
		for classifier in classifier_list:
			for cohorte_filter in filter_list:
				print "\n => Control vs "+str(cohorte_filter)+"\n"
				max_iteration = 1000
				number_of_individual_per_generation = 350
